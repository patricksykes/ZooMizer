#' Set constants for zooplankton model
#'
#' @param params TODO Patrick: document this
#' @param Groups 
#' @param sst 
#'
#' @return A MizerParams object with constants set for the zooplankton model
...
newZooMizerParams <- function(groups, input, fish_params) {
  ...
  params@other_params$temp_eff <-  matrix(2.^((sst - 30)/10), nrow = length(params@species_params$species), ncol = length(params@w))
  
  params <- setZooMizerConstants(params = params, Groups = groups, sst = input$sst)
  #params@initial_n[] <- readRDS("data/initialn.RDS")
  ...
  #params@search_vol[] <- readRDS("data/SearchVol.rds")
  #params <- setSearchVolume(params)
  #params <- setmort_test(params, sst)
  # M_sb <- getExtMort(params)
...
resource_zooMizer <- function(params, n_other, ...) {
  ...
  zoo_idx <- (length(zoo_params@w_full) - length(zoo_params@w) + 1):length(zoo_params@w_full)
  total_n_eff <- n_pp * zoo_params@other_params$assim_phyto[1] / params@species_params$alpha[1] *  0 #NOTE: this means no fish feeding on phyto
