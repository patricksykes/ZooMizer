...
fish_params <- newTraitParams(no_sp = 5,
                              ...
                              kappa = 10^(input$phyto_int) * 10^-2,
                              lambda = 2.2, # 1 - input$phyto_slope,
...
#input$tmax <- 100
#stable_zoo <- fZooMizer_run(groups = groups, input = input)

#stable_zoo <- readRDS("test_grid_20210317.RDS")[[ID]] #needs to be the same w grid etc...
...
for (i in 1:nrow(new)) {
  new[i,w_min_idx[i]:(w_max_idx[i])] <- spline(zoomizer_w, y = stable_zoo[i,], method = "fmm",
                                            ...
                                            ties = mean
                                            )$y[w_min_idx[i]:(w_max_idx[i])] / zoo_params@dw[w_min_idx[i]:(w_max_idx[i])]
}

fish_params@initial_n_other$zoo <- new
