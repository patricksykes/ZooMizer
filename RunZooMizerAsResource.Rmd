...
## FROM ZOOMIZER
stable_zoo <- readRDS("test_grid_20210317.RDS")[[ID]]

intercept <- mean(getCommunitySlope(stable_zoo, species = c("Fish_Small", "Fish_Med", "Fish_Large"), )$intercept[501:1000])
slope <- mean(getCommunitySlope(stable_zoo, species = c("Fish_Small", "Fish_Med", "Fish_Large"))$slope[501:1000])

f <- function(x) exp(intercept) * x^(slope)
plotSpectra(stable_zoo, species = c("Fish_Small", "Fish_Med", "Fish_Large"))+geom_function(fun = f)


times <- length(getTimes(stable_zoo))
stable_zoo <- colSums(stable_zoo@n[ceiling(times/2):times,,])
...
melt(N(sim)) %>% 
    ...
    layout(xaxis = list(type = "log", exponentformat = "power"),
           yaxis = list(type = "log", exponentformat = "power", range = c(-20, 1)))
...
steady_fish <-  steady(fish_params, t_max = 200, return_sim = TRUE, dt = 0.1, t_per = 1.5)
