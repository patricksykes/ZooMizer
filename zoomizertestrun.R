...
timeseries <- foreach(ID = 1:20) %dopar% {
  source("ZooMizerPlots.R")
  source("ZooMizerSummaryFunctions.R")
  sims[[ID]]@params@linetype[1:nrow(species_params(sims[[ID]]@params))] <- "dashed"
  plotBiomass_ZooMizer(sims[[ID]], zoo_params)+
    labs(title = paste0("SST = ", enviro$sst[ID], ", chlo = ", round(enviro$chlo[ID],3)))
}

library(patchwork)
timeseriesgrid <- wrap_plots(timeseries, nrow =  5, ncol = 4) + plot_layout(guides = "collect")
# timeseriesgrid
ggsave(paste0("Plots/",jobname,"_timeseries_gridplot.png"), timeseriesgrid, width = 20, height = 24)

# biomasses <- foreach(i=1:length(sims), .combine = rbind) %dopar% sum(colMeans(tail(getBiomass(sims[[i]]),25)))

# spectra plots
spectra <- foreach(i = 1:20) %do% {
  source("ZooMizerPlots.R")
  source("ZooMizerSummaryFunctions.R")
  tmax  <- max(as.numeric(dimnames(sims[[i]]@n)$time))
  time_range = ceiling(3/4*tmax+1):tmax
  plotSpectra_ZooMizer(sims[[i]], zoo_params, time_range = time_range, wlim = c(1e-14, NA), total = TRUE)+
...
ggsave(paste0("Plots/",jobname,"_spectra_gridplot.png"), spectplots, width = 20, height = 24)


# Diet plots

params <- foreach(ID=1:20) %dopar% {
  params <- setInitialValues(sims[[ID]]@params,sims[[ID]])
  times <- max(getTimes(sims[[ID]]))
  params@initial_n[] <- colMeans(sims[[ID]]@n[ceiling(times/2+1):times,,])
  nothers <- array(0,c(times,dim(params@initial_n_other[[1]])))
  for (i in 1:times) {
    nothers[i,,]  <- sims[[ID]]@n_other[[i]]
  }
  params@initial_n_other$zoo[] <- colMeans(nothers[ceiling(times/2+1):times,,])
  params@initial_n_pp <- colMeans(sims[[ID]]@n_pp[ceiling(times/2+1):times,])
  params
}

dietplots <- foreach(ID = 1:20) %dopar% {
  source("ZooMizerPlots.R")
  source("ZooMizerSummaryFunctions.R")
  plotDiet_ZooMizer(params[[ID]], species = "13")+
    labs(title = paste0("SST = ", enviro$sst[ID], ", chlo = ", round(enviro$chlo[ID],3)))
}

library(patchwork)
dietsgrid <- wrap_plots(dietplots, nrow =  5, ncol = 4) + plot_layout(guides = "collect")
# timeseriesgrid
ggsave(paste0("Plots/",jobname,"_diets_gridplot.png"), dietsgrid, width = 24, height = 24)

# # basic diet plots (all plankton = resource)
# basicdietplots <- foreach(ID = 1:20) %dopar% {
#   plotDiet(params[[ID]], species = "13")+
#     labs(title = paste0("SST = ", enviro$sst[ID], ", chlo = ", round(enviro$chlo[ID],3)))
# }

# basicdietsgrid <- wrap_plots(basicdietplots, nrow =  5, ncol = 4) + plot_layout(guides = "collect")
# # timeseriesgrid
# ggsave(paste0("Plots/",jobname,"_dietsbasic_gridplot.png"), basicdietsgrid, width = 24, height = 24)

#get realised PPMRs
rPPMRsprop <- foreach(ID = 1:20, .combine = rbind) %dopar% {
  # simSample <- params[[ID]]
  # pred_kernel(simSample) <- pred_kernel(simSample)
  getRealisedPPMRs(params[[ID]])
}
