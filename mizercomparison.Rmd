---
title: "Adjusting ZooMizer reproduction"
...
jobname <- "20220531_15deg_chlovarying_reprolevel0_alphapt6_1000yr"
...
foreach(ID=1:25, .combine = rbind) %do% sims[[ID]]@params@species_params$erepro
...
slopes <- foreach(ID=c(1:25), .combine = rbind) %dopar% { 
  colMeans(getCommunitySlope(sims[[ID]])[751:1000,], na.rm = TRUE)
}
...
Now compare with a run of Mizer with a semichemostat resource:

```{r}
mizerjobname <- '202200608_mizertest_alphapt6_rl0_zoocutoff100'
```


```{r semichemostat, eval  = FALSE}

mizerwalltimes <- foreach(ID = 1:25,
                .combine = rbind,     
                .packages = c("mizer", "assertthat", "tidyverse"),
                .export = c("jobname", "enviro")
) %dopar% {
...
ZooMizer_mizercomp <- function(ID, tmax = 1000, effort = 0) {
  groups <- read.csv("data/TestGroups_mizer.csv") #load in ZooMSS groups
  ...
  fish_params <- newTraitParams(no_sp = 13,
                                ...
                                no_w = 10*(7-(-3))+1, # sets dx = 0.1
                                min_w_pp = 10^(-14.5),
                                w_pp_cutoff = 100, #corresponds to max jellyfish size of 100g in ZooMizer
                                alpha = 0.6, 
                                kappa = exp(intercept), #10^(input$phyto_int) * 10^-1,
                                lambda = 1 - slope, # 1 - input$phyto_slope,
                                gamma = 640 * temp_eff, #takes care of temp effect on PredRate and Encounter
...
out <- ZooMizer_mizercomp(ID, tmax = 100, effort = 0)
saveRDS(out, file = paste0("Output/", mizerjobname, "_Mizer_", ID_char,".RDS"))
...
```{r mizercombine}
msims <- foreach(ID = 1:25,
                .packages = c("mizer", "assertthat", "tidyverse")
                #.export = c("ZooMizer_coupled", "PredRate_temp")
) %dopar% {
  ID_char <- sprintf("%04d",ID)
  readRDS(paste0("Output/", mizerjobname, "_Mizer_", ID_char,".RDS"))
}
...
mslopes <- foreach(ID=c(1:25), .combine = rbind) %dopar% { 
  colMeans(getCommunitySlope(msims[[ID]])[76:100,], na.rm = TRUE)
}


mslopes <- cbind(enviro, mslopes)

mslopesplot <- ggplot(mslopes, aes(x=log10(chlo)))+
  geom_point(aes(y=slope))
mintsplot <- ggplot(mslopes, aes(x=log10(chlo)))+
  geom_point(aes(y=intercept))

mslopesplot / mintsplot + plot_annotation(title = "Fish community slope + intercept (Mizer only)")
...
zoomssjobname <- "20220519_15deg_chlovarying"
zsims <- foreach(ID = 1:25,
                .packages = c("mizer", "assertthat", "tidyverse")
                #.export = c("ZooMizer_coupled", "PredRate_temp")
) %dopar% {
  ID_char <- sprintf("%04d",ID)
  readRDS(paste0("Output/", zoomssjobname, "_ZooMSS_", ID_char,".RDS"))
}

zslopes <- foreach(ID=c(1:25), .combine = rbind) %dopar% { 
  colMeans(getCommunitySlope(zsims[[ID]], species = c("Fish_Small", "Fish_Med", "Fish_Large"))[751:1000,], na.rm = TRUE)
}

zslopes <- cbind(enviro, zslopes)

zslopesplot <- ggplot(zslopes, aes(x=log10(chlo)))+
  geom_point(aes(y=slope))
zintsplot <- ggplot(zslopes, aes(x=log10(chlo)))+
  geom_point(aes(y=intercept))

zslopesplot / zintsplot + plot_annotation(title = "Fish community slope + intercept (ZooMSS fish only)")

```


## Compare biomass across the three models

```{r biomscomparison}

zoomizerbioms <- foreach(ID=1:25, .combine=rbind) %dopar% sum(colMeans(getBiomass(sims[[ID]])[751:1000,]))
mizerbioms <- foreach(ID=1:25, .combine=rbind) %dopar% sum(colMeans(getBiomass(msims[[ID]])[76:100,]))
zoomssbioms <- foreach(ID=1:25, .combine=rbind) %dopar% sum(colMeans(getBiomass(zsims[[ID]])[751:1000,]))

bioms.df <- cbind(enviro, zoomizerbioms, mizerbioms, zoomssbioms)

zmcol <- ggplot(bioms.df, aes(x=chlo))+
  geom_col(aes(y=zoomizerbioms))+
  scale_x_continuous(trans = "log10")

zcol <-  ggplot(bioms.df, aes(x=chlo))+
  geom_col(aes(y=zoomssbioms))+
  scale_x_continuous(trans = "log10")

mcol <-  ggplot(bioms.df, aes(x=chlo))+
  geom_col(aes(y=mizerbioms))+
  scale_x_continuous(trans = "log10")

zmcol+mcol+zcol+plot_annotation(title = "Comparison of ZooMizer, Mizer, ZooMSS fish biomass per m^3")

```
## Mizer with fixed zooplankton resource

```{r semichemostat, eval  = FALSE}

mizerfixedjobname <- '202200608_mizertest_alphapt6_rl0_zoocutoff100_fixedzoo'

mizerfixedwalltimes <- foreach(ID = 1:25,
                .combine = rbind,     
                .packages = c("mizer", "assertthat", "tidyverse"),
                .export = c("jobname", "enviro")
) %dopar% {
...
phyto_fixed <- function(params, n, n_pp, n_other, rates, dt, kappa, lambda, ... ) {
  npp <- params@initial_n_pp
 return(npp)
}

ZooMizer_mizerfixed <- function(ID, tmax = 1000, effort = 0) {
  groups <- read.csv("data/TestGroups_mizer.csv") #load in ZooMSS groups
  ...
  fish_params@resource_params$w_pp_cutoff <- 100
fish_params@initial_n_pp <- fish_params@resource_params$kappa*fish_params@w_full^(-fish_params@resource_params$lambda)
fish_params@initial_n_pp[params@w_full > 100] <- 0
fish_params <- setResource(fish_params, resource_dynamics = "phyto_fixed")
...
out <- ZooMizer_mizerfixed(ID, tmax = 100, effort = 0)
saveRDS(out, file = paste0("Output/", mizerfixedjobname, "_MizerFixed_", ID_char,".RDS"))
...
mfsims <- foreach(ID = 1:25,
                .packages = c("mizer", "assertthat", "tidyverse")
                #.export = c("ZooMizer_coupled", "PredRate_temp")
) %dopar% {
  ID_char <- sprintf("%04d",ID)
  readRDS(paste0("Output/", mizerfixedjobname, "_MizerFixed_", ID_char,".RDS"))
}

mfslopes <- foreach(ID=c(1:25), .combine = rbind) %dopar% { 
  colMeans(getCommunitySlope(mfsims[[ID]])[76:100,], na.rm = TRUE)
}

mfslopes <- cbind(enviro, mfslopes)

mfslopesplot <- ggplot(mfslopes, aes(x=log10(chlo)))+
  geom_point(aes(y=slope))
mfintsplot <- ggplot(mfslopes, aes(x=log10(chlo)))+
  geom_point(aes(y=intercept))

mfslopesplot / mfintsplot + plot_annotation(title = "Fish community slope + intercept (Mizer only)")

```

Now compare biomass with other versions

```{r mizerfixedcompare}

mizerfixedbioms <- foreach(ID=1:25, .combine=rbind) %dopar% sum(colMeans(getBiomass(mfsims[[ID]])[76:100,]))

mf.df <- cbind(enviro, mizerfixedbioms)

mfcol <- ggplot(mf.df, aes(x=chlo))+
  geom_col(aes(y=mizerfixedbioms))+
  scale_x_continuous(trans = "log10")

zmcol+mcol+zcol+mfcol+plot_annotation(title = "Comparison of ZooMizer, Mizer, ZooMSS, fixed-resource Mizer fish biomass per m^3")
