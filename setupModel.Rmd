...
mf.params <- setZooMizerConstants(params = mf.params, Groups = groups, input_params = enviro_row)
...
mf.params@initial_n_pp <- mf.params@resource_params$kappa * mf.params@w_full^(1 - mf.params@resource_params$lambda)
mf.params@initial_n_pp[mf.params@w_full>mf.params@resource_params$w_pp_cutoff] <- 0
...
zoomssgrid <- readRDS("zoomssgrid.rds")
zoomssgrid <- zoomssgrid[enviro$CellID]
...
for (i in 1:length(zoomssgrid)) {
   hack@n[i,,]<- zoomssgrid[[i]]
   pzoomss[[i]] <- plotSpectra(hack, time_range = i, power = 0)+labs(title = paste("ZooMSS plot",i))
}

w <- matrix(hack@params@w, nrow = nrow(hack@params@species_params), ncol = length(hack@params@w))

#load in ZooMizer data and compare
zoomizergrid <- readRDS("test_grid.RDS")

pzoomizer <- list()
for (i in 1:length(zoomizergrid)) {
  zoomizergrid[[i]]@n[1001,,] <- apply(as.array(zoomizergrid[[i]]@n[502:1001,,]),c(2,3),'mean') #apples-to-apples comparison
  pzoomizer[[i]] <- plotSpectra(zoomizergrid[[i]], time_range = 1000, power = 0)+labs(title = paste("ZooMizer plot",i))
}

pzoomss[[20]]
pzoomizer[[20]]

testdf <- data.frame(zoomsstcb = numeric(20),zoomizertcb = numeric(20))
for (i in 1:nrow(testdf)) {
testdf$zoomsstcb[i] = sum(hack@n[i,,]*w)
testdf$zoomizertcb[i] = sum(zoomizergrid[[i]]@n[1001,,]*w)
}
...
testdf$chlo <- enviro$chlo
(chlotcb <- ggplot(data=testdf, aes(x=log10(chlo),y=log10(zoomsstcb)))+
  geom_point())
