...
Something is not quite right...Here's a very hacky way to get the comparison...I've run the model for 20 different combinations of SST and chlorophyll-a and matched them to the relevant runs of ZooMSS (without diffusion).
...
#dim(zoomizergrid[[1]]@n)

pzoomizer <- list()
for (i in 1:20) {
  zoomizergrid[[i]]@n[1001,,] <- apply(as.array(zoomizergrid[[i]]@n[502:1001,,]),c(2,3),'mean')
...
for (i in 1:20) {
testdf$zoomsstcb[i] = sum(hack@n[i,,])
testdf$zoomizertcb[i] = sum(zoomizergrid[[i]]@n[1001,,])
}

ggplot(testdf, aes(x=log10(zoomsstcb), y=log10(zoomizertcb)))+
  geom_point()+
