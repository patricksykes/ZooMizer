...
mf.params@other_params$temp_eff <-  matrix(2.^((enviro_row$sst - 30)/10), nrow = length(mf.params@species_params$species), ncol = length(mf.params@w))
...
Now, to recreate this in mizer, we need to rewrite mizer's `project_simple()` function. We do this by making a new function, `new_project_simple()`, and using it in place of the default one. Note that there are two key changes in here: to change the boundary conditions ("reproduction") and to account for the conversion in abundance units from absolute abundance in ZooMSS to normalised abundance in ZooMizer.
