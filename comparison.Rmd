...
all.equal(A_iter, a, check.attributes = FALSE)
...
e_growth_phyto <- new_EReproAndGrowth(params = params2, n = params2@initial_n, n_pp = params2@initial_n_pp, n_other = params2@initial_n_other, t = 0, encounter = getEncounter(params2), feeding_level = getFeedingLevel(params2))
a_phyto <- matrix(0, nrow = no_sp, ncol = no_w)
a_phyto[, idx] <- sweep(e_growth_phyto[, idx - 1, drop = FALSE] * dt, 2,
                      params@dw[idx], "/")
all.equal(a_phyto[, idx], param$dt/param$dx * model$ingested_phyto[, idx-1])
