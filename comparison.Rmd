...
no_sp <- nrow(params@species_params) # number of species
  ...
  mort <- getMort(params)
    ...
    a[, idx] <- sweep(e_growth[, idx - 1, drop = FALSE] * dt , 2,
                      params@dw[idx]*params@w[idx], "/") *0.1 /log(10)
    # b_{ij} = 1 + g_i(w_j) / dw_j dt + \mu_i(w_j) dt
    b[] <- 1 + sweep(e_growth * dt , 2, params@dw*params@w, "/")* 0.1 * 10^0.1 / log(10) + mort * dt * 0.1
    # S_{ij} <- N_i(w_j)
...
all.equal(getExtMort(params), zoomsstest$model$M_sb, check.attributes = FALSE) # 10 times higher - covered in the numerical step
