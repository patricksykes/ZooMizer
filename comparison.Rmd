...
all.equal(zoomsstest$model$nPP,
          zoomizertest@n_pp[1,1:length(zoomsstest$model$param$w_phyto)] *
            zoomizertest@params@dw_full[1:length(zoomsstest$model$nPP)], check.attributes=FALSE)
...
idx.iter <- 2:ngrid
  ...
  # N <- matrix(zoomizertest@n[1,,], nrow = ngrps, ncol = ngrid) 
    ...
    M2 <- .colSums(colSums(ap2),param$ngrid,param$ngrid) # 1 x n_sizes
    ...
    A.iter[,idx.iter] <- param$dt / param$dx * gg[,idx.iter-1] # Growth stuff
    C.iter[,idx.iter] <- 1 + param$dt * Z[,idx.iter] + param$dt/param$dx * gg[,idx.iter] # Mortality
    ...
    C[,idx] <- 1 + param$dt * Z[,idx] + param$dt/param$dx*(gg[,idx] + diff[,idx] * (log(10)/2+1/param$dx)) # Mortality
    S[,idx] <- N[,idx]
...
no_sp <- nrow(params@species_params) # number of species
  ...
  mort <- getMort(params)
    ...
    b[] <- 1 + sweep(e_growth * dt, 2, params@dw, "/") + mort * dt
    # S_{ij} <- N_i(w_j)
    S[,idx] <- n[, idx, drop = FALSE]

    
all.equal(e_growth, gg, check.attributes = FALSE)
all.equal(A.iter, a, check.attributes = FALSE)
all.equal(C.iter, b, check.attributes = FALSE)
...
all.equal(getExtMort(params), zoomsstest$model$M_sb, check.attributes = FALSE) # TRUE

#predation mortality
all.equal(getPredMort(zoomizertest@params)[1,], M2, check.attributes = FALSE)

#overall mortality
all.equal(getMort(params), Z, check.attributes = FALSE)
